...
// --- GM to Minecraft Note Block Mapping (simplificado)
const gmToNoteBlock = {
  ...
  41:{block:"Flute", baseNote:6-12}, 42:{block:"Flute", baseNote:6-12}, 46:{block:"Harp", baseNote:12},
...
function secondsToTicks(seconds, bpm){ return Math.round(seconds * 20); }
...
async function processMidi(){
  ...
  midi.tracks.forEach(track=>{
    track.notes.forEach(note=>{
      ...
      const isPercussion = note.midi>=35 && note.midi<=81; 
  ...
  // Sort ticks and calculate delta ticks
  const sortedTicks = Object.keys(ticksMap).map(Number).sort((a,b)=>a-b);
  let html=`<table><tr><th>Tick</th><th>Delta to next tick</th><th>Notes</th></tr>`;
  
  for(let i=0;i<sortedTicks.length;i++){
    const tick = sortedTicks[i];
    const nextTick = sortedTicks[i+1] !== undefined ? sortedTicks[i+1] : tick;
    const delta = nextTick - tick;
    const notesStr = ticksMap[tick].map(n=>`${n.block} (${n.note})`).join(", ");
    html+=`<tr><td>${tick}</td><td>${delta}</td><td>${notesStr}</td></tr>`;
  }
